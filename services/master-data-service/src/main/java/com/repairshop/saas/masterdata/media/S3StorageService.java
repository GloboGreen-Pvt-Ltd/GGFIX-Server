...
public class S3StorageService {
    ...
    /**
     * Upload one object by STREAMING the multipart part straight through to S3.
     *
     * The byte-array overload above is right for catalogue artwork, which is capped at
     * a few megabytes. It is wrong for a 40 MB device video on a 160 MB heap: that
     * path needs the whole file resident, and a couple of concurrent uploads is an
     * OutOfMemoryError. Spring has already spooled the part to disk, so streaming it
     * costs a file handle and a small copy buffer regardless of file size.
     *
     * The content length is passed explicitly rather than left to the SDK — without
     * it the client buffers the stream itself to discover the length, which reinstates
     * exactly the problem this overload exists to avoid.
     *
     * @return the public URL the object is now reachable at
     * @throws MediaStorageException if S3 is unconfigured or the put fails
     */
    public String put(String key, MultipartFile file, String contentType) {
        requireEnabled();
        ...
        try (InputStream body = file.getInputStream()) {
            ...
            log.info("Uploaded s3://{}/{} ({} bytes, {}, streamed)",
                    props.getBucket(), key, length, contentType);
            return props.publicUrl(key);
        } catch (S3Exception e) {
            String detail = e.awsErrorDetails() == null ? e.getMessage() : e.awsErrorDetails().errorMessage();
            throw new MediaStorageException("S3 upload failed for key '" + key + "': " + detail, e);
        } catch (IOException e) {
            throw new MediaStorageException("Could not read the uploaded file for key '" + key + "'.", e);
        } catch (RuntimeException e) {
            throw new MediaStorageException("S3 upload failed for key '" + key + "'.", e);
        }
    }
