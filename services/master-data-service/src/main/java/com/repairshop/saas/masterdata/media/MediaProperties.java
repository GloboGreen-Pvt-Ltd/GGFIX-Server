...
public class MediaProperties {
    ...
    /**
     * Per-video ceiling in bytes, an order of magnitude above the image one: the
     * booking flows record up to 30 seconds of device coverage, which a modern phone
     * camera will happily make 30 MB of. Videos are streamed to S3 rather than
     * buffered, so this ceiling costs disk in the servlet's multipart spool, not heap.
     */
    private long maxVideoBytes = 48L * 1024 * 1024;

    /**
     * Per-recording ceiling in bytes. Voice notes are a customer describing a fault,
     * not a podcast — a minute of AAC is well under a megabyte, so 16 MB is generous
     * and still small enough that a runaway recording cannot fill the spool.
     */
