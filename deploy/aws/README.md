...
- Java 21
- Dockerized PostgreSQL 16 (unused when `DB_HOST` points at RDS)
...
| File | Purpose |
| --- | --- |
| `install-ec2.sh` | One-time host prep: Java, Docker, app user, `/opt/repair-shop-saas` |
| `deploy-from-artifact.sh` | Installs jars from the bundle, starts Postgres, restarts units |
...
API_BASE=http://13.205.198.41 bash deploy/aws/health-check.sh    # direct IP, pre-TLS
...
1. **A record** — `api.ggfix.in` → the EC2 public IP, TTL 300. certbot's HTTP-01
   ...
   The workflow verifies this and stops early if the record and `AWS_EC2_HOST`
   disagree.
...
The first deploy creates `/opt/repair-shop-saas/.env`. Edit it for RDS
credentials, Cloudinary keys, or to trim `SERVICES`:

```bash
sudo nano /opt/repair-shop-saas/.env
```

`SERVICES` defaults to all twelve. A t3.micro cannot hold twelve JVMs — shorten
the list there, and mirror it in `OPTIONAL_SERVICES` so the health gate agrees.

## GitHub secrets

| Secret | Required | Notes |
| --- | --- | --- |
| `AWS_EC2_HOST` | yes | Public IP or DNS of the deploy target |
| `AWS_EC2_USER` | no | Defaults to `ec2-user` (Amazon Linux); `ubuntu` on Ubuntu AMIs |
| `AWS_EC2_SSH_KEY` | yes | Private key whose public half is in the host's `authorized_keys` |
| `AWS_EC2_PORT` | no | Defaults to `22` |
...
failed-unit logs, the nginx config test, and disk usage from the host.
